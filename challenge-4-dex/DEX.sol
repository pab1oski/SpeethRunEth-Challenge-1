...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        require(totalLiquidity == 0, "DEX: it is already initilized");
        totalLiquidity = address(this).balance;
        liquidity[msg.sender] = totalLiquidity;
        require(
            token.transferFrom(msg.sender, address(this), tokens),
            "transfer not transact"
        );
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        require(msg.value > 0, " Cannot tranfer 0");
        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 token_Reserve = token.balanceOf(address(this));
        tokenOutput = price(msg.value, ethReserve, token_Reserve);

        require(token.transfer(msg.sender, tokenOutput), "Failed Swap eth-tok");
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0, "cannot swap 0");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        require(msg.value > 0, "you cannot add 0");
        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 token_Reserve = token.balanceOf(address(this));

        tokensDeposited = (msg.value.mul(token_Reserve) / ethReserve).add(1);
        uint256 liquidityMint = msg.value.mul(totalLiquidity) / ethReserve;
        liquidity[msg.sender] = liquidity[msg.sender].add(liquidityMint);
        totalLiquidity = totalLiquidity.add(liquidityMint);

        // require(token.balanceOf(msg.sender) >= tokensDeposited);
        // require(token.approve(address(this), tokensDeposited));
    ...
    function withdraw(
        ...
        require(liquidity[msg.sender] >= amount, "not enought");
